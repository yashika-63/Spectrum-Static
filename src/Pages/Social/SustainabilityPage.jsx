import React, { useState } from "react";
import "../../CSS/AboutUs/NewsPage.css";
import NewsCard from "../Components/NewsCard";
import { useNavigate } from "react-router-dom";
import { sustainData } from "../../Data/NewsData";

const SustainabilityPage = () => {
  const [searchTerm, setSearchTerm] = useState("");
  const [yearFilter, setYearFilter] = useState("");

  const navigate = useNavigate();

  const years = [...new Set(sustainData.map((item) => item.date.split("/")[2]))];

  const filteredSustain = sustainData.filter((item) => {
    const matchYear = yearFilter ? item.date.endsWith(yearFilter) : true;
    const matchSearch =
      item.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.description.toLowerCase().includes(searchTerm.toLowerCase());
    return matchYear && matchSearch;
  });

  const handleCardClick = (sustains) => {
    navigate(`/Sustainability/${sustains.id}`);
  };

  return (
    <div className="news-page">
      <h2 className="title">Sustainability Initiatives</h2>
      <div className="news-header">
        <div className="filters">
          <input
            type="text"
            placeholder="Search Sustainability..."
            onChange={(e) => setSearchTerm(e.target.value)}
          />
          {/* <select
            onChange={(e) => setYearFilter(e.target.value)}
            value={yearFilter}
          >
            <option value="">All Years</option>
            {years.map((year) => (
              <option key={year} value={year}>
                {year}
              </option>
            ))}
          </select> */}
        </div>
      </div>

      <div className="news-grid">
        {filteredSustain.length ? (
          filteredSustain.map((news) => (
            <NewsCard key={news.id} news={news} onClick={handleCardClick} />
          ))
        ) : (
          <p>No Sustainability events found.</p>
        )}
      </div>
    </div>
  );
};

export default SustainabilityPage;
